...
## Reliability Analysis for Community data

#reliability_report <- paste0("raw_alpha: Cronbach’s $\alpha$ (values ≥ .7 or .8 indicate good reliability; Kline (1999))")

reliability_tools_community_stats_all <- if (length(unique(selected_vars_df$tool_reliability[selected_vars_df$tool_reliability != "none" &
                                ...
                                function(x){
                                  ...
                                  reliability <- if (nn %in% reliability_rescale) {
                                    psych::alpha(df_analysis %>%
                                                   dplyr::filter(.data[[group_vars]] == "Community") %>%
...
reliability_tools_community_stats_group <- if (length(unique(selected_vars_df$tool_reliability_group[selected_vars_df$tool_reliability_group != "none" &
...
correlation_tools_community_stats <- if (length(unique(selected_vars_df$tool_correlation[selected_vars_df$tool_correlation != "none" &
