...
## Cleaning the university data

df_university_clean <- df_university_raw_final %>%
  dplyr::rename(any_of(university_new_variable_names)
                ) %>%
  dplyr::mutate(education = "Tertiary level"
                ...
                , study = gsub(" Year", "", study)
                , study = factor(study, levels = c("First", "Second", "Third", "Fourth")
                                 ) # encode study column to factor
                ...
                , supportive = factor(supportive, levels = c("Not suppotive", "Slightly supportive", "Moderately Supportive",
                                                             "Supportive", "Very Supportive")
                                      ) # encode supportive column to factor
                , incomesource = factor(incomesource, levels = c("Family", "Personal", "Loan", "Scholarship", "Other")
                                        ) # encode incomesource column to factor
                ...
                , across(c(sex, h_head, awareness, mh_services, psq_psychosis, psq_hypomania:psq_hallucinations2),
                         ~as.factor(.x)
